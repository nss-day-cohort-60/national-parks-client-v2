...
export const BlogFilter = ({
  ...
  const blogSearchAndFilterFetcher = (park_id, searchTerm) => {
    return fetch(
      `http://localhost:8000/blogs?park_id=${park_id}&key_word=${searchTerm}`
    )
      .then((res) => res.json())
      .then((data) => blogSetterFunction(data));
  };
    ...
    return (
      <section className="row">
        <div className="col-sm-12 col-md-6 d-flex flex-column">
          ...
          <select
            className="filter"
            onChange={(e) => setPark_id(e.target.value)}
          >
            <option key={`park--0`} value={0}>
              All Parks
            </option>
            {parks.map((park) => {
              return (
        ...
        <div className="col-sm-12 col-md-6 d-flex flex-column">
          ...
          <input
            className="search"
            onChange={(e) => {
              searchSetterFunction(e.target.value);
            }}
